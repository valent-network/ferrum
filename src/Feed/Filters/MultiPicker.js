...
function MultiPicker({filters, applyFilter, opt: {localized_name, name, values}}) {
  ...
  return <View>
    ...
    <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
      {values.sort((a, b) => a.position - b.position).map(filterValue => {
        const onPress = () => applyFilter(name, filterValue);
        const active = activeFilters.filter((f) => f == filterValue.id).length === 1;
        const iconName = activeFilters.filter((f) => f == filterValue.id).length === 1
          ? 'checkmark-circle-outline'
          : 'ellipse-outline';

        return <MultiPickerItem key={`filter-${filterValue.id}`} filterValue={filterValue.name} filterId={filterValue.id} onPress={onPress} active={active} iconName={iconName} />
      })}
    </ScrollView>
  </View>
}
